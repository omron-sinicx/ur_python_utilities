...
def gazebo_callback(msg):
    ...
    for i, obj_name in enumerate(msg.name):
        if obj_name.endswith("robot"):
            ...
            world[:3] -= robot[:3]
            world[:3] = np.quaternion(np.roll(robot[3:],1)).inverse * world[:3]
            world[3:] = np.roll((np.quaternion(axis=[1.0, 0.0, 0.0], degrees=-90).inverse * np.quaternion(np.roll(world[3:],1))).elements,-1)
            world[3:] = transformations.quaternion_rotate_vector(transformations.quaternion_inverse(transformations.quaternion_from_euler(0, 0, 0)), world[3:])
